...
pub fn run(input: &str) {
    ...
    // Last two tokens are min and max
    let min_str = parts[parts.len() - 2];
    let max_str = parts[parts.len() - 1];

    let min: f64 = match min_str.parse() {
        Ok(v) => v,
        Err(_) => {
    ...
    let max: f64 = match max_str.parse() {
        Ok(v) => v,
        Err(_) => {
    ...
    // Everything before min/max is the formula
    ...
    // Sample 1000 points across the range
    let steps = 1000;
